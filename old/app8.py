...
if 'game_phase' not in st.session_state:
    ...
    # Initialize the custom cast editor with 6 random characters
...
if st.session_state.game_phase == 'setup':
    ...
    st.write("Edit the names and personalities directly in the table below. Add or delete rows to change the number of players (Min: 4, Max: 12). You can add your friends, celebrities, or entirely new archetypes!")
    
    col1, col2 = st.columns([3, 1])
    with col2:
        # A quick way to reset the table with a new random assortment
    ...
    if st.button("Begin the Mystery", use_container_width=True):
        if len(edited_cast) < 4:
            ...
            # Keep track of names to prevent duplicates if user accidentally typed the same name twice
            used_names = []
