...
def clean_llm_output(text, speaker_name, roster_names):
    # Aggressively remove hallucinated backend instructions
    leak_patterns = [r"Instruction.*", r"CRITICAL RULES.*", r"Transcript so far.*", r"You are.*", r"CRITICAL:.*", r"CRITICAL RULES:.*"]
    for p in leak_patterns:
        text = re.sub(p, "", text, flags=re.IGNORECASE | re.DOTALL)
        
    # Remove Narration (anything inside asterisks or brackets)
    text = re.sub(r'\*.*?\*', '', text)
    text = re.sub(r'\[.*?\]', '', text)
    text = re.sub(r'\(.*?\)', '', text)
    ...
    # Remove accidental name prefixes (e.g., "Arthur the Butler:", "Arthur:")
    text = text.strip()
    for name in roster_names + [speaker_name]:
        full_prefix = name.lower() + ":"
        short_prefix = name.split()[0].lower() + ":"
        if text.lower().startswith(full_prefix):
            text = text[len(name)+1:].strip()
        elif text.lower().startswith(short_prefix):
            text = text[len(name.split()[0])+1:].strip()
...
# --- 3. DAY PHASE ---
    ...
    if st.session_state.speaker_idx < len(st.session_state.daily_order):
        ...
        if st.button(f"Let {current_speaker} Speak", use_container_width=True):
            ...
            # The secret role logic is abstracted out of the system prompt to prevent declaration
            if player_data['role'] == 'Killer':
                sys_p += " Deflect blame onto someone else in the room."
            elif player_data['role'] == 'Detective':
                sys_p += " Ask a suspicious question to someone in the room."
                if st.session_state.god_whispers:
                    sys_p += f" Focus heavily on this clue: {st.session_state.god_whispers[-1]}"
...
# --- 4. STRICT DETECTIVE EVALUATION ---
    ...
    # We lock the guess in session_state so clicking buttons doesn't overwrite it!
    if 'current_guess' not in st.session_state:
        with st.spinner("The hidden Detective is reviewing the evidence..."):
            ...
            st.rerun() # Rerun immediately to display the locked guess

    # Retrieve locked guess
