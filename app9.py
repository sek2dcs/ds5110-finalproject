...
# --- CONFIGURATION & LORE ---
AVAILABLE_MODELS = ['llama3:8b'] # Keeping it to one model to prevent memory crashes!
...
def clean_llm_output(text, speaker_name, roster_names):
    # 1. Strip instructions and prompt leaks completely
    leak_patterns = [
    ...
    # 2. Strip physical actions and quotation marks
    text = re.sub(r'\*.*?\*', '', text)
    text = re.sub(r'\[.*?\]', '', text)
    text = re.sub(r'\(.*?\)', '', text)
    ...
    # 3. Aggressive Name Stripper (Fixes "Martha the Cook: Julian: Marissa...")
    changed = True
    while changed:
        orig = text
        # Regex looks for up to 4 capitalized words followed by a colon
        text = re.sub(r'^([A-Z][a-zA-Z]+(?:\s+[A-Za-z]+){0,3}):\s*', '', text).strip()
        # Fallback for exact speaker match just in case
...
if st.session_state.game_phase == 'setup':
    ...
    st.write("Edit the names and personalities directly in the table below. Add or delete rows to change the number of players (Min: 4, Max: 12).")
...
# --- 5. GOD PHASE ---
    ...
    # NEW logic to pause the screen on the success message
    if not st.session_state.whisper_cast:
        ...
        if st.button("Continue to Night Phase", use_container_width=True):
            st.session_state.whisper_cast = False # Reset for the next day
